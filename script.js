...
function GetWeather(city = "Washington") {
  var city_changer_value = document.getElementById("city-changer").value;
  if (city_changer_value && city_changer_value != city) {
    city = city_changer_value;
  }
  var city_el = document.getElementById("city");
  var current_temp = document.getElementById("temp");
  $.getJSON(
    "https://api.weatherapi.com/v1/current.json?key=62cff90ddeca4f6f818160332202812&q=" +
      city,
    function (
      data //Get all needed data from API
    ) {
      console.log(data);
      var current_weather = data.current.condition["text"];
      var current_temp_c = data.current.temp_c; //Get current temperature in C
      city_el.innerHTML = city; //Set the current city
      current_temp.innerHTML = current_temp_c + "C"; //Set the current temp
      ...
      if (
        ...
        current_weather == "Light freezing rain" ||
        current_weather == "Light rain"
      ) {
        changeWeather(weather[2]);
      } else if (current_weather == "Patchy heavy snow") {
        changeWeather(weather[0]);
      }
      //And so on
    }
